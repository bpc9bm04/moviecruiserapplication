...
public class MovieServiceImplTest {
	...
	@Before
	public void setUp() {
		movie = new Movie(1, 101, "Avengers", "Nice Movie", "www.movieworld.com", "06-06-2019", "userId");
	...
	public void testSaveMovieFailure() throws MoviewAlreadyExistException {
		when(movieRepositoryMock.findByUserIdAndMovieId(movie.getUserId(), movie.getMovieId())).thenReturn(movie);
	...
	public void testUpdateMovieSuccess() throws MovieNotFoundException {
		...
		verify(movieRepositoryMock, times(1)).findByUserIdAndMovieId(movie.getUserId(),movie.getMovieId());
		verify(movieRepositoryMock, times(1)).save(movie);
	}
	...
	public void testDeleteMovieByIdSuccess() throws MovieNotFoundException {
		when(movieRepositoryMock.findByUserIdAndMovieId(movie.getUserId(),movie.getMovieId())).thenReturn(movie);
		boolean movieDeleted = movieServiceImpl.deleteMovieById(movie.getUserId(),movie.getMovieId());
		assertTrue("Movie deleted successfully", movieDeleted);
		verify(movieRepositoryMock, times(1)).findByUserIdAndMovieId(movie.getUserId(),movie.getMovieId());
		verify(movieRepositoryMock, times(1)).deleteByUserIdAndMovieId(movie.getUserId(),movie.getMovieId());
	}
	...
	public void testDeleteMovieByIdFailure() throws MovieNotFoundException {
		movieServiceImpl.deleteMovieById(movie.getUserId(),movie.getMovieId());
	}
	...
	public void testGetMovieByIdSuccess() throws MovieNotFoundException {
		when(movieRepositoryMock.findById(movie.getMovieId())).thenReturn(options);
		movieServiceImpl.getMovieById(movie.getMovieId());
		verify(movieRepositoryMock, times(1)).findById(movie.getMovieId());
	}
	...
	public void testGetAllMovies() {
		...
		allMovies.add(movie);
		when(movieRepositoryMock.findByUserId("userId")).thenReturn(allMovies);
		List<Movie> Movies = movieServiceImpl.getMyMovies("userId");
		assertEquals(Movies, allMovies);
		verify(movieRepositoryMock, times(1)).findByUserId("userId");
	}
}
